...
public class ProduitDAO_XML {
	...
	public boolean creer(I_Produit p) {
		try {
			...
			Element nomcategorie = new Element("nomcategorie");
			prod.addContent(nomcategorie.setText(String.valueOf(p.getNomCategorie())));
	...
	public I_Produit lire(String nom) {
		...
		if (e != null) {
			I_Categorie categorie = GestionnaireCategorie.getCategory(e.getAttributeValue("nomcategorie"));
			return new Produit(e.getAttributeValue("nom"), Double.parseDouble(e.getChildText("prixHT")), Integer.parseInt(e.getChildText("quantite")), e.getAttributeValue("nomcatalogue"), categorie);
		}
		else
			return null;
	}
	...
	public List<I_Produit> lireTous(String nomCatalogue) {
		...
		try {
	        ...
	        for(int i = 0 ; i < nodes.getLength() ; i++){
				...
				String nomCategorie = nodes.item(i).getAttributes().item(3).getTextContent();
				I_Categorie categorie = GestionnaireCategorie.getCategory(nomCategorie);
				l.add(new Produit(nomP, prix, qte, nomCatalogue, categorie));
			}
		} catch (Exception e) {
